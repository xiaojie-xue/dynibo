...
|---|---|---|
| Rust | 每个 `Robot` 持有一个 workspace | 矩阵和广义力 buffer 由调用方提供 |
...
每个 `Robot` 实例持有与其不可变模型绑定的 workspace。`fork()` 会共享模型、创建新的
计算存储。

## 并行计算

每个 Rust `Robot` 都是可变的，同一时刻只能参与一次计算。并行计算时应为每个任务调用
`fork()` 创建实例。Python 会串行化同一个 `Robot` 上的调用，需要并行时应使用独立 robot。
